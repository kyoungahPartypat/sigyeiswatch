...
function rowLeft(map, i, j){
  ...
  if(i-1 >= 0){
    ...
    if(row === 3 && sWall >= 0 && map[x][j] === "black"){
      row++;
    }
  }else{
    row =0;
  ...
  if(i-1 >= 0 && i+1 < map.length){
    for(var n=0; n<3; n++){
      if(isWall < map.length && map[x][j] === "white"){
		  return 0;
	  }else if(sWall < map.length &&map[x][j] === "black"){
        row++;
      }
  ...
  if(j-1 >= 0 && j+1 < map[i].length){
    for(var n=0; n<3; n++){
      if(isWall < map[i].length && map[i][y] === "white"){
	    return 0;
	  }else if(sWall < map[i].length  && map[i][y] === "black"){
        console.log(isWall);
        cell++;
      }
   ...
   //로비 채팅
    ...
    socket.on("gameSet", function(data){ 
      ...
      if(rooms[room].status === "대기중"){
        if(data.userList ==  0 /*rooms[room].count*/){
    ...
    socket.on('firstTurn', function(data){
      var room = people[user];
      var wake = 0;
      socket.emit('moveNight', {game:game[room], myJob:data.myJob, wake:wake});
      console.log('first');
    });
    ...
    socket.on('showMinion', function(data){
      ...
      if(game[rooms].ware <= 0){
        ...
        socket.emit('systemMssage', {msg:text});
      }
    });
      ...
      if(people[user] != undefined && lobby[user] === undefined){
        ...
        if(rooms[room].people.length == 0){
          ...
          if(rooms[room].owner == user){
            ...
            io.sockets.in(room).emit('systemMessage', {user:rooms[room].owner, msg:"님이 방장이 되었습니다."});
          }
