...
WareS.prototype.deadJob = function(findJob){
  var index = -1;

  for(var i=0; i<this.dead.length; i++){
    if(this.dead[i].job === findJob){
      return 1;
    }else{
      index = index;
    }
  ...
  io.on('connection', function(socket){
   ...
   //로비 채팅
          ...
          if(rooms[room].type === "warewolf"){
            ...
            switch(rooms[room].count){
              case '3':
                ...
                break;
              case '5':
                var type = ['늑대인간', '늑대인간', '천리안', '도둑', '문제아', '주정뱅이', '마을사람', '마을사람'];
                var wake = 4;
                break;
              case '7':
                var type = ['늑대인간', '늑대인간', '미니언', '천리안', '도둑', '문제아', '주정뱅이', '불면증 환자', '마을사람', '마을사람'];
                var wake = 5; 
              case '10':
                var type = ['도플갱어', '늑대인간', '늑대인간', '미니언', '천리안', '비밀요원', '비밀요원',  '도둑', '문제아', '주정뱅이', '불면증환자', '마을사람', '마을사람']
                var wake = 6;
                break; 
              default :
                break;
            ...
            for(var i = 0; i < rooms[room].count; i++){
              ...
              if(type[i] === "늑대인간"){
                game[room].ware.push(game[room].people[i].name);  
              }else if(type[i] === "비밀요원"){
                game[room].mason.push(game[room].people[i].name);
              }

              game[room].people[i].job = type[i];
              j++;
            }
            console.log(game[room].ware.length);
            for(var i = 0; i < 3; i++){
    ...
    socket.on('nextTurn',function(data){
      ...
      }else if(wake == game[room].wake){
        socket.emit('systemMessage', {msg:"움직임이 사라졌습니다."});
    ...
    socket.on('showSeer', function(data){
      ...
      if(user != "noUser"){
        ...
        if(num != -1){
          var job = game[room].people[num].job;
    ...
    socket.on('showRobber', function(data){
      var room = people[user];
      var sUser = data.user;
      var sNum = game[room].search(sUser);
      var sJob = game[room].people[sNum].job; //선택한 사람의 직업
      var mNum = game[room].search(user);
      var mJob = game[room].people[mNum].job; //도둑
      var tmp = sJob; //선택한 사람의 직업을 먼저 임시 변수에 담고 

      game[room].people[sNum].job = game[room].people[mNum].job //선택한 사람의 직업을 도둑으로 바꿈
      game[room].people[mNum].job = tmp //내직업을 선택한 사람의 직업으로 바꿈
	  
      socket.emit("systemMessage", {msg:"당신의 직업은 " + tmp + "입니다"});
    });
    ...
    socket.on('showTrouble', function(data){
      ...
      var temp = game[room].people[num].job; //첫번째 유저 직업

      game[room].people[num].job = game[room].people[num2].job;
      game[room].people[num2].job = temp;

      console.log(game[room].people);
    });
    ...
    socket.on('showDrunk', function(data){
      ...
      var num = data.num;
      var mNum = game[room].search(user);
      var temp = game[room].people[mNum].job //주정뱅이

      game[room].people[mNum].job = game[room].card[num];
      game[room].card[num] = temp;

      console.log(game[room].people[mNum]);
     
      socket.emit('systemMessage', {msg:"당신의 직업이 바꼈습니다."}); 
    });
    ...
    socket.on('checkVote', function(data){
      ...
      if(game[room].status == "대기중"){
        ...
        if(!game[room].isVote){
          ...
          io.sockets.in(room).emit('systemMessage', {msg:"투표를 시작합니다. 의심되는 사람을 지목해주세요."});
        }
      }
    });
    ...
    socket.on('skipVote', function(data){
      ...
      if(rooms[room].status == "대기중"){
        socket.emit('systemMessage', {msg:"게임중이 아닙니다."});
      }else if(game[room].isVote){
        socket.emit('systemMessage', {msg:"투표 중 입니다."});
      }else{
        if(!game[room].daytime){
          socket.emit('systemMessage', {msg:"아직 스킵버튼을 누를 수 없습니다."});
        }else{
          if(!game[room].isVote){
    ...
    socket.on("voteUser", function(data){
      ...
      if(game[room].vCount === rooms[room].people.length){
        ...
        if(game[room].dead.length === game[room].people.length){
          text+= "없습니다.";
        }else{
          for(var i = 0; i < game[room].dead.length; i++){
            
            if(i === game[room].dead.length -1){
              text += game[room].dead[i].name;
            }else{
              text += game[room].dead[i].name + ",";
            }
          }
        ...
        io.sockets.in(room).emit('systemMessage', {msg:text});
        io.sockets.in(room).emit('gameResult', {});
      }
    ...
    socket.on('showResult', function(data){
      ...
      for(var i = 0; i < game[room].people.length; i++){
        console.log(game[room].people);
        if(i === game[room].people.length -1){
          text+= game[room].people[i].name + ": " + game[room].people[i].job;
        }else{
          text+= game[room].people[i].name + ": " + game[room].people[i].job + ", ";
        }
      ...
      if(game[room].dead.length === game[room].people.length){  //죽은 사람이 아무도 없을 때 
        if(game[room].ware.length === 0){
          socket.emit("systemMessage", {msg:"시민팀 승리!"}); 
        }else{
          socket.emit("systemMessage", {msg:"늑대팀 승리!"}); //늑대가 아무도 안죽었으므로 
        }
      }else{  //죽은사람이 한명이라도 있을 때 
        var num = game[room].deadJob("늑대인간");
        
        if(num === 1){
          socket.emit("systemMessage", {msg:"시민팀 승리!"}); //늑대가 한마리라도 죽었으므로 
	}else{
          num = game[room].deadJob("무두장이");

          if(num === 1){
            socket.emit("systemMessage", {msg:"무두장이 승리!"}); //무두장이가 죽었으므로
 	  }else{
            socket.emit("systemMessage", {msg:"늑대팀 승리!"}); //늑대가 아무도 안죽었으므로 
          }
        }
