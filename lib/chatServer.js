...
function checkPointer(map, turn,  i,  j){
  ...
  var x = i-1;
  var y = j-1;
  var rowCount = 1, cellCount = 1, lDCount = 1, rDCount = 1;

  map[i][j] = turn;
 
  //↖
  while(x >= 0 && y >= 0 && map[x][y] === map[i][j]){
    lDCount++;
    x -=1;
    y -=1;
  }
  ...
  //↗
  while(x >= 0 && y < map[i].length && map[x][y] === map[i][j]){
    rDCount++;
    x -= 1;
    y += 1;
  }
  ...
  //→ 
  while(y < map[i].length && map[x][y] === map[i][j]){
    cellCount ++;
    y += 1;
  }
  ...
  //↙
  while(x < map.length && y >= 0 && map[x][y] === map[i][j]){
    rDCount++;
    x +=1;
    y -=1;
  }
  ...
  //↓
  while(x < map.length && map[x][y] === map[i][j]){
    rowCount++; 
    x +=1;
  }
  ...
  //↘
  while(x < map.length && y < map[i].length && map[x][y] === map[i][j]){
    lDCount++;
    x +=1;
    y +=1;    
  }

  if(rowCount === 5 || cellCount === 5 || lDCount === 5 || rDCount === 5){
    win = turn;
  }
  ...
  io.on('connection', function(socket){
   ...
   //로비 채팅
    ...
    socket.on('checkPoint', function (data){
      ...
      if(rooms[room].status != "대기중"){
        console.log(game[room].map[data.x][data.y]);
    ...
    socket.on('compulsionTurn', function(data){
      ...
      io.sockets.in(room).emit('cTurn', {turn:data.turn});     
    });
    //
          ...
          if(rooms[room].status === "게임중"){
            if(rooms[room].type === "omok"){
              if(game[room].black === user){
                io.sockets.in(room).emit('result', {result:'white'});  
                rooms[room].status = "대기중";
              }else{
                io.sockets.in(room).emit('result', {result:'black'});  
                rooms[room].status = "대기중"; 
              }
            }else{
